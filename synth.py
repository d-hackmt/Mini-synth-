...
# ---------------- Utility Functions ---------------- #
def adsr_envelope(total_samples, A, D, S, R, sustain_level=0.7):
    attack = np.linspace(0, 1, int(A*total_samples), endpoint=False)
    decay = np.linspace(1, sustain_level, int(D*total_samples), endpoint=False)
    sustain = np.ones(int(S*total_samples)) * sustain_level
    release = np.linspace(sustain_level, 0, int(R*total_samples), endpoint=False)
    return np.concatenate((attack, decay, sustain, release))

def generate_wave(freq, duration, sample_rate=44100, instrument="piano"):
    t = np.linspace(0, duration, int(sample_rate * duration), endpoint=False)

    if instrument == "piano":
        wave = np.sin(2 * np.pi * freq * t) * np.exp(-3 * t)  
    elif instrument == "flute":
        wave = np.sin(2 * np.pi * freq * t) * np.exp(-0.5 * t)  
    elif instrument == "trumpet":
        wave = (np.sin(2*np.pi*freq*t) + 0.3*np.sin(4*np.pi*freq*t)) * np.exp(-2*t)
    elif instrument == "guitar":
        wave = np.sin(2*np.pi*freq*t) * np.exp(-2*t) * np.sign(np.sin(2*np.pi*6*t))
    else:
...
def note_to_freq(note):
    ...
    octave = int(note[-1])
    key = notes[note[:-1]]
    return A4 * 2**((key + (octave-4)*12)/12)

# ---------------- Streamlit UI ---------------- #
st.title("🎹 Python Synth + Generative Music")

tab1, tab2 = st.tabs(["Sound Design Fundamentals", "Generative Sequences"])

# ---------------- TAB 1 ---------------- #
with tab1:
    st.header("ADSR Synth Playground")

    A = st.slider("Attack", 0.01, 1.0, 0.1)
    D = st.slider("Decay", 0.01, 1.0, 0.2)
    S = st.slider("Sustain", 0.01, 1.0, 0.5)
    R = st.slider("Release", 0.01, 1.0, 0.3)
    ...
    if st.button("Play Sound"):
        ...
        wave = np.sin(2 * np.pi * np.linspace(0, duration, total_samples) * freq)
        env = adsr_envelope(total_samples, A, D, S, R)
        sound = wave[:len(env)] * env

        sound = np.int16(sound / np.max(np.abs(sound)) * 32767)
        buffer = io.BytesIO()
        write(buffer, sample_rate, sound)
        st.audio(buffer, format="audio/wav")

        fig, ax = plt.subplots()
        ax.plot(env[:2000])  
        ax.set_title("ADSR Envelope Visualization")
        st.pyplot(fig)

# ---------------- TAB 2 ---------------- #
with tab2:
    st.header("Generative Music Sequences")

    scale_type = st.selectbox("Choose Scale", ["Major", "Minor"])
    chord_prog = st.selectbox("Chord Progression", ["I-IV-V-I", "ii-V-I", "vi-IV-I-V", "Random"])
    tempo = st.slider("Tempo (BPM)", 60, 180, 120)
    duration = st.slider("Sequence Duration (seconds)", 10, 20, 15)
    instrument = st.selectbox("Instrument", ["piano", "flute", "trumpet", "guitar"])
    ...
    if st.button("Generate Sequence"):
        ...
        if scale_type == "Major":
            ...
            scale = ["C4", "D4", "D#4", "F4", "G4", "G#4", "A#4", "C5"]

        if chord_prog == "I-IV-V-I":
            chords = [["C4","E4","G4"], ["F4","A4","C5"], ["G4","B4","D5"], ["C4","E4","G4"]]
        elif chord_prog == "ii-V-I":
            chords = [["D4","F4","A4"], ["G4","B4","D5"], ["C4","E4","G4"]]
        elif chord_prog == "vi-IV-I-V":
            chords = [["A4","C5","E5"], ["F4","A4","C5"], ["C4","E4","G4"], ["G4","B4","D5"]]
        else:
            chords = [[random.choice(scale) for _ in range(3)] for _ in range(4)]
        ...
        while current_sample < total_samples:
            ...
            chord_wave = sum(generate_wave(note_to_freq(n), beat_duration, sample_rate, instrument) for n in chord)
            chord_wave /= len(chord)

            end_sample = current_sample + len(chord_wave)
            if end_sample > total_samples:
                break
            music[current_sample:end_sample] += chord_wave
            current_sample = end_sample

        music = np.int16(music / np.max(np.abs(music)) * 32767)
        buffer = io.BytesIO()
        write(buffer, sample_rate, music)
        st.audio(buffer, format="audio/wav")

        fig, ax = plt.subplots()
        ax.plot(music[:5000])
        ax.set_title("Generated Music Waveform")
        st.pyplot(fig)
